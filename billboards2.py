...
new_hotspots = new_hotspots[new_hotspots['name']=='shaggy-felt-shark']
...
def stats(city_name):
    ...
    cols = ['name','location','city', 'status','day earnings', 'month earnings','total mined','reward scale','closest hotspot (m)','recent witnesses', 'link']
    return df[cols]
...
#         hot_data['quartile'] = hot_data['name'].map(df.to_dict()['quartile'])
        ...
        #hot_data.at['TOTAL','reward scale'] = ' '
        #hot_data.at['TOTAL','closest hotspot (m)'] = ' '
        ...
        #s = hot_data.style.applymap(color_status, subset=['status', 'closest hotspot (m)'])
        #hot_data = hot_data.apply(lambda x: ['background: lightsteelblue' if x.name == 'TOTAL' else '' for i in x], axis=1)
